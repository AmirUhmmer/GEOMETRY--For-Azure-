...
// #region 360 Zones
export async function zone360(viewer, message) {
  viewer.clearSelection();
  ...
  if (!model3) {
    console.warn("Third model not found");
    return;
  }
  ...
  ------------------------------*/
  console.log("Building / using FL cache for model:", functionalLocationCache);
  if (!functionalLocationCache.has(model3.id)) {
    functionalLocationCache.set(model3.id, new Map());
  }

  const locationMap = functionalLocationCache.get(model3.id);
  ...
  tenantGroups.forEach(({ tenant, locations }, index) => {
    ...
    const themeColor =
      tenant === "Unoccupied"
        ? red
        : tenantToThemeColor(tenant);

    const selectColor =
      tenant === "Unoccupied"
        ? redSel
        : new THREE.Color(themeColor.x, themeColor.y, themeColor.z);
    ...
    tenantDbIds.forEach(id =>
      viewer.setThemingColor(id, themeColor, model3)
    );
  ...
  viewer.impl.invalidate(true);
  viewer.setGhosting(true);
  viewer.select(finalSelection, model3);
}
