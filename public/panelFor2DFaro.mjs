...
export function ToolbarButton2DFaro(viewer, modelAbbreviation) {
  ...
  showFaro2DPanelButton.setToolTip('2D View');  // Set the tooltip for the button
  ...
  gatherAll2DFiles(models).then((all2DFiles) => {
      ...
      showFaro2DPanelButton.onClick = function() {
          if (viewer.Faro2DPanel) {
              ...
              showFaro2DPanel(viewer, all2DFiles, modelUrn);  // Pass the all2DFiles array to your custom function
          }
      };
  });
 ...
 // Function to create and display a docking panel
 export async function showFaro2DPanel(viewer, all2DFiles, modelUrn) {
  class Faro2DPanel extends Autodesk.Viewing.UI.DockingPanel {
    constructor(viewer, title, options) {
      super(viewer.container, title, options);
    ...
    createScrollContainer() {
      ...
      this.scrollContainer.style.paddingBottom = '-50%';  // Add padding to the scroll container
      this.scrollContainer.style.height = '100%';  // Ensure it takes full panel height calc(100% + 2em)
    ...
    createPanelContent() {
      ...
      content.forEach(item => {
          ...
          if (item.viewableID) {
            ...
            //urn:adsk.wipemea:fs.file:vf.xdXReqV0T1azoWueEiSnzg?version=67
            ...
            clickableItem.onclick = () => {
              ...
              //const modelUrn = btoa(models[0].urn); // Assuming the first model is the one you want to load
              //const modelUrn = btoa('urn:adsk.wipemea:fs.file:vf.xdXReqV0T1azoWueEiSnzg?version=55')
              //const modelUrn = 'dXJuOmFkc2sud2lwZW1lYTpmcy5maWxlOnZmLnhkWFJlcVYwVDFhem9XdWVFaVNuemc_dmVyc2lvbj02Nw';
              const access_token = localStorage.getItem('authToken');
              //console.log('URN:', urnPart); // Log the access token for debugging
              ...
              async function onDocumentLoadSuccess(doc, item) {
                ...
                try {
                  ...
                  // console.log("Default Geometry:", defaultGeometry); // Log the default geometry for debugging
